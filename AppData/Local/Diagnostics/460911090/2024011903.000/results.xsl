...
function idof(key)
{
  ...
  a{ color: #0066CC; }
  button{color: transparent; background-color: transparent; border-color: transparent; outline: 0;}
  .page { width: 480px; }
  ...
  .local{ text-decoration: none; font-size: 9pt; }
...
function folder(d, state)
{
  try
  {
    ...
    var button = document.getElementById("expandButton_" + d.id);
  ...
  if (window.event.keyCode == 13) {
    ...
    event.returnValue = false;
						...
						<div class="page">
							...
							<button class="button" print="false" tabindex="0" onclick="window.print();" onkeydown="key_print();">
								...
								<div style="display: 'none';"><xsl:value-of select="'print'"/></div>
							</button>
		...
		<xsl:if test="count(msxsl:node-set($rootcause))">
			<table class="info block" cellpadding="0" cellspacing="0" style="margin-top: 12px;">
				<xsl:variable name="tablename">
					<xsl:value-of select="msxsl:node-set($strings)/String[@id = $title][1]/child::node()"/>
				</xsl:variable>
				<xsl:attribute name="aria-label"><xsl:value-of select="concat($tablename,' ', 'table')"/></xsl:attribute>
						...
						<td>
							<div nowrap="true" class="clip">
								...
								<a class="local">
									...
									<xsl:attribute name="onfocus">style.textDecoration='underline';</xsl:attribute>
									<xsl:attribute name="onblur"><xsl:value-of select="$popup"/>.style.display='none'; style.textDecoration='none';</xsl:attribute>
				...
				<td>
					<table class="info" cellpadding="0" cellspacing="0">
						...
						<tr>
							<td width="20px">
								<xsl:if test="$details = 'true'">
									<div role="button" class="arrows" aria-expanded="false" estyle="padding-top: 2px; width: 14px; height: 14px;" tabindex="0" bullet="true">
										...
										<xsl:attribute name="aria-label"><xsl:value-of select="'expand'"/></xsl:attribute>
				...
				<td>
					<table class="info" cellpadding="0" cellspacing="0">
						...
						<tr>
							<td width="20px">
								<xsl:if test="$details = 'true'">
									<div role="button" class="arrows" aria-expanded="false" style="padding-top: 2px; width: 14px; height: 14px;" tabindex="0" bullet="true">
										<xsl:attribute name="onkeydown">key_folder(c_<xsl:value-of select="$tag"/>);</xsl:attribute>
										<xsl:attribute name="id">e_c_<xsl:value-of select="$tag"/></xsl:attribute>
										<xsl:attribute name="aria-label"><xsl:value-of select="'expand'"/></xsl:attribute>
										<xsl:attribute name="altText">5</xsl:attribute>
			...
			<table class="info" cellpadding="0" cellspacing="0" style="border-bottom: solid 1px lightgrey;">
				<tr style="padding-bottom: 3px;">
					...
					<td>
						...
						<xsl:choose>
							<xsl:when test="$tag">
								<button class="button" id="button1" aria-expanded="false" tabindex="0" print="false">
									<xsl:attribute name="id">expandButton_c_<xsl:value-of select="$tag"/></xsl:attribute>
									...
									<div style="display: 'none';"><xsl:value-of select="'expand'"/></div>
								</button>
							</xsl:when>
							<xsl:otherwise>
								<xsl:attribute name="style">padding-top: 9px; font-size: 9pt;</xsl:attribute>
								<a class="local" onmouseover="style.textDecoration='underline';" onmouseout="style.textDecoration='none';" onfocus="style.textDecoration='underline';" onblur="style.textDecoration='none';" print="false">
